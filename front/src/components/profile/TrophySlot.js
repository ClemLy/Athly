...
export function TrophySlot({ trophy, onPress }) {
...
// onUnfeature optionnel : absent → lecture seule (vitrine d'un ami)

export function FeaturedModal({ trophy, onClose, onUnfeature }) {
          ...
          <Pressable style={styles.modalInner} onPress={() => {}}>
            ...
            {unlocked && (
              <View style={[styles.badge, { backgroundColor: color + '1A', borderColor: color + '55' }]}>
                ...
                <Text style={[styles.badgeText, { color }]}>{onUnfeature ? 'Débloqué · En vitrine' : 'Débloqué'}</Text>
              </View>
            )}
